...
- Interactive terminals use arrow keys, Space, and Enter to select update types and cooldowns. The default is minor and patch updates; protected singleton upgrades default to `Yes`. Choosing Major first shows its candidate table, then opens a separate package selector with nothing preselected, including from `All`. Cooldown-held packages are shown before their exception selector; protected packages declined by the user are never offered as exceptions.
- `--types=major,minor,patch` selects exact SemVer categories, `--types=all` selects every category, and `--major`/`--latest` remain compatibility aliases for all categories. With `--yes`, those major flags explicitly authorize unattended major updates. Unknown, duplicate value, and contradictory options fail instead of silently changing the selected policy.
- `--dry-run` renders the preview without changing manifests, overrides, or lockfiles. Interactive dry runs also walk through cooldown exceptions and show their resulting preview before exiting; `--yes` remains unattended. No selector or apply confirmation appears when no update can be applied.
- Upgrade previews and final reports group major, minor, and patch changes separately. Repeated package, change, and release values are shown once per consecutive workspace group. Final reports always include skipped updates and their reason, including after selecting cooldown exceptions.
