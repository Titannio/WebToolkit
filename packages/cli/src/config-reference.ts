...
export const configSchema: JsonSchema = {
  ...
  $defs: {
    ...
    e2eServer: {
        type: 'object',
        required: ['name', 'command', 'readinessUrl', 'timeoutMs'],
        additionalProperties: false,
        properties: {
          ...
          cwd: projectPath('Optional project-relative working directory.'),
          env: { type: 'object', additionalProperties: { type: 'string' } },
          readinessUrl: { type: 'string', minLength: 8, pattern: '^https?://' },
      ...
      e2eTests: {
        type: 'object',
        description: 'Playwright end-to-end test runner and managed services.',
        required: ['playwrightPackage', 'testDirectory', 'browser', 'playwright', 'runner', 'servers'],
        additionalProperties: false,
        properties: {
          playwrightPackage: { type: 'string', minLength: 1, default: '@playwright/test' },
          testDirectory: projectPath('Directory containing Playwright specs.'),
          testFilePattern: { type: 'string', format: 'regex', default: '\\.spec\\.(ts|tsx|js|jsx)$' },
          ...
          playwright: {
            ...
            properties: {
              ...
              ciConfig: { type: 'object', additionalProperties: { $ref: '#/$defs/jsonValue' } },
            },
          },
          runner: {
            type: 'object',
            ...
            properties: {
              command: { type: 'string', minLength: 1 },
              args: stringArray('Playwright runner arguments.'),
            },
          },
          servers: { type: 'array', minItems: 1, items: { $ref: '#/$defs/e2eServer' } },
        },
        examples: [{
          playwrightPackage: '@playwright/test',
          testDirectory: 'tests/e2e',
          browser: 'chromium',
          playwright: { config: { testMatch: '**/*.spec.ts', use: { baseURL: 'http://localhost:3000' } } },
