...
describe('config file discovery', () => {
  ...
  it('rejects invalid config before downstream dispatch', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'webtoolkit-config-'))
    const configDirectory = path.join(root, '.webtoolkit-cli')
    await mkdir(configDirectory, { recursive: true })
    await writeFile(path.join(configDirectory, 'config.json'), JSON.stringify({
      repoCheck: { steps: [{ label: 'Missing runner' }] },
    }), 'utf8')
    const dispatch = vi.fn()

    await expect(loadConfig(root).then(dispatch)).rejects.toThrow('repoCheck.steps[0]')
    expect(dispatch).not.toHaveBeenCalled()

    await writeFile(path.join(configDirectory, 'config.json'), '{', 'utf8')
    await expect(loadConfig(root)).rejects.toThrow('Invalid .webtoolkit-cli/config.json')
    await rm(root, { recursive: true, force: true })
  })
})
...
describe('config merging', () => {
  it('merges cleaner overrides while preserving defaults', () => {
    ...
    expect(config.guards?.internalLink?.includePaths).toEqual(['apps/web/src'])
