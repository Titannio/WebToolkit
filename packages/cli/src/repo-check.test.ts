...
vi.mock('./guard-runner.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('./guard-runner.js')>()
  return {
    ...actual,
...
describe('repo check engine', () => {
  ...
  it('validates every step before executing any command', () => {
    const invalidConfigs = [
      { label: 'Missing runner' },
      { label: 'Ambiguous runner', command: 'pnpm', builtinGuard: 'any' },
      { label: 'Unknown guard', builtinGuard: '__proto__' },
    ]

    for (const invalidStep of invalidConfigs) {
      expect(() => runRepoCheck(runtimeWithConfig('/repo', {
        packageManager: 'pnpm',
        repoCheck: {
          steps: [
            { label: 'Valid', command: 'pnpm' },
            invalidStep as never,
          ],
        },
      }))).toThrow()
    }

    expect(runCommandInherited).not.toHaveBeenCalled()
    expect(executeBuiltinGuard).not.toHaveBeenCalled()
  })
  ...
  it('reports execution errors as failures and skips later fail-fast steps', () => {
    const consoleInfo = vi.spyOn(console, 'info').mockImplementation(() => undefined)
    const processExit = vi.spyOn(process, 'exit').mockImplementation(() => undefined as never)
    vi.mocked(runCommandInherited).mockImplementation(() => {
    ...
    runRepoCheck(runtimeWithConfig('/repo', {
      ...
      repoCheck: {
        failFast: true,
        steps: [
          { label: 'Broken', command: 'pnpm' },
          { label: 'Later', command: 'pnpm' },
        ],
      },
    }))

    expect(runCommandInherited).toHaveBeenCalledTimes(1)
    expect(consoleInfo).toHaveBeenCalledWith(expect.stringMatching(/Broken[\s\S]*FAIL[\s\S]*Later[\s\S]*SKIP/))
    expect(processExit).toHaveBeenCalledWith(1)
    processExit.mockRestore()
  })
})
