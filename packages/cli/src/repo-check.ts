...
function validateSteps(steps: TaskStepConfig[]): void {
  for (const step of steps) {
    ...
    if (step.builtinGuard && !Object.hasOwn(builtinGuards, step.builtinGuard)) {
      throw new Error(
        `Unknown builtin guard "${step.builtinGuard}". Available guards: ${Object.keys(builtinGuards).sort().join(', ')}.`,
      )
    }
  }
}
...
export function runRepoCheck(runtime: Runtime): void {
  ...
  for (const step of config.steps) {
    ...
    try {
      ...
      console.error(`\n${colorize(`[FAIL] ${step.label}: ${(error as Error).message}`, `${colors.bright}${colors.red}`)}`)
    }
  }
